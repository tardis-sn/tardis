...
One can then use the configuration file as one normally would! (For new users, see the :ref:`quickstart guide <quickstart/quickstart>` to see how to use a configuration file. See also the :ref:`Custom TARDIS Model Tutorial <Custom_TARDIS_Model_Tutorial>`.)
