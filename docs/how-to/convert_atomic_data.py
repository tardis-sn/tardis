...
filename = "merged_mod_20SNG_forbidden_yg_fix_H30_cmfgen_yg.h5"
old = h5py.File(filename)

# Photoionization data stored in separate file in Christian's version
pi_filename = "photoionization_data_H30_He.h5"
pi_data = pd.HDFStore(pi_filename)

# Reference atomic data file for the format we want to convert to
template = pd.HDFStore(
    "/home/connor/tardis-regression-data/atom_data/nlte_atom_data/TestNLTE_He_Ti.h5"
)

# Open up a new pandas HDFStore to port the old data into
basename, ext = filename.split(".")
new_filename = f"{basename}_CONVERTED.{ext}"
