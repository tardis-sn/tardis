...
class MacroAtomTransitionType(IntEnum):
    ...
    # Currently unused transitions - bf and ff emission are implemented below by channel
    BF_EMISSION = -2
    FF_EMISSION = -3
    ADIABATIC_COOLING = -4
...
def macro_atom_interaction_iip(
    ...
    abs_matrix_source_indices = opacity_state.abs_matrix_source_indices
    abs_matrix_indices = opacity_state.abs_matrix_indices

    current_transition_type = 0
    while current_transition_type >= 0:
        if activation_level_idx == 0:
            raise (ValueError)
        if activation_level_idx in abs_matrix_source_indices:
            absorbing_state_probability = 0.0
            for to_state_index, state_probability in enumerate(
                opacity_state.absorbing_markov_probabilities[
                    current_shell_id,
                    np.searchsorted(
                        abs_matrix_source_indices, activation_level_idx
                    ),
                ]
            ):
                ...
                if absorbing_state_probability > probability_event:
                    absorbing_activation_level_idx = abs_matrix_source_indices[
                        np.searchsorted(abs_matrix_indices, to_state_index)
                    ]
                    if absorbing_activation_level_idx == 0:
                        raise (ValueError)
                    break
            else:
                raise MacroAtomError(
        ...
        for deactivation_channel_index in range(
            ...
            if emission_transition_probability > probability_emission_event:
                ...
                emission_id = opacity_state.transition_line_id[
                    deactivation_channel_index
                ]
