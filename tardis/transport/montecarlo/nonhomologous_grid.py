...
def depressed_quartic(
    ...
    aybw_thresh = threshold_fac * max(
        np.abs(3.0 * a), np.abs(2.0 * y), np.abs(2.0 * b / w)
    )
    ...
    if np.isnan(aybw_term_clamped[0]) and np.isnan(aybw_term_clamped[1]):
        if np.abs(aybw_term[0]) <= np.abs(aybw_term[1]):
            ...
            minroot_ind = 1
        thresh_match = (
            np.abs(aybw_term[minroot_ind]) / aybw_thresh * threshold_fac
        )
        ...
        if verbose:
            print(
                ...
                threshold_fac,
                "and would need to be greater than",
                thresh_match,
                "to catch this case.",
            )
