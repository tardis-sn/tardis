...
def geometry(config):
    ...
    return parse_geometry_configuration.parse_geometry_from_config(config,time_explosion)
...
class TestValidFields:
    ...
    def test_calculate_mean_intensity_sim_state(self, simulation_state_rad_field, atom_dataset, regression_data):
        nu = atom_dataset.lines["nu"].values
        actual_intensities = np.array(simulation_state_rad_field.calculate_mean_intensity(nu))
        expected_intensities = regression_data.sync_ndarray(actual_intensities)
        npt.assert_array_equal(actual_intensities, expected_intensities)
    ...
    def test_dilute_factors_len_equals_temp_len_geometry(self, geometry_rad_field):
        assert len(geometry_rad_field.dilution_factor) == len(geometry_rad_field.temperature)

    def test_calculate_mean_intensity_geometry(self, geometry_rad_field, atom_dataset, regression_data):
        nu = atom_dataset.lines["nu"].values
