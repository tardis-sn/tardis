...
__all__ = ['StimulatedEmissionFactor', 'TauSobolev', 'BetaSobolev',
           'TransitionProbabilities', 'BaseRadBoundBoundTransProbs']

C_EINSTEIN = (4. * (np.pi * const.e.esu) ** 2 / (
    const.c.cgs * const.m_e.cgs)).value * u.eV.to(u.erg)
...
class BaseRadBoundBoundTransProbs(TransitionProbabilities):
    ...
    def calculate(self, atomic_data, beta_sobolev, j_blues,
        ...
        p_rad_bb = super().calculate(atomic_data, beta_sobolev, j_blues,
                                     stimulated_emission_factor, tau_sobolevs)
        ...
        mask_continuum_species = pd.MultiIndex.from_arrays(
            [atomic_data.macro_atom_data.atomic_number,
             atomic_data.macro_atom_data.ion_number]).isin(
                 continuum_interaction_species)
