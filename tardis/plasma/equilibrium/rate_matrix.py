...
class RateMatrix:
    ...
    def prepare_species_rates(
        ...
        collisional_species_mask = (
            ...
            collisional_rates.index.get_level_values("ion_number") == species[1]
        )
        ...
        collisional_rates = (
            ...
            .to_numpy(dtype=np.float64)
            * thermal_electron_energy_distribution.number_density.value[None, :]
        )
...
def assemble_ion_rate_matrices(
    ...
    atomic_numbers = (
        aggregated_rates[0].index.get_level_values("atomic_number").unique()
    )
    ...
    for atomic_number_idx, atomic_number in enumerate(atomic_numbers):
        rate_matrix_array[atomic_number_idx] = list(
            rate_matrices[atomic_number]
        )
    return (
        pd.DataFrame(
            rate_matrix_array,
...
class EstimatedIonRateMatrix:
    ...
    def __init__(
        ...
        self.prepared_matrix_coefficients: (
            dict[int, tuple[npt.NDArray[np.float64], ...]] | None
        ) = None
    ...
    def prepare(
        ...
        matrix_coefficients: dict[int, tuple[npt.NDArray[np.float64], ...]] = {}
        atomic_numbers = (
            aggregated_rates[0].index.get_level_values("atomic_number").unique()
        )
        for atomic_number in atomic_numbers:
        ...
        if effective_lte_ionization_factor is not None:
            for (
                atomic_number
            ) in effective_lte_ionization_factor.index.get_level_values(
                "atomic_number"
            ).unique():
                ...
                for shell_idx, shell in enumerate(level_population.columns):
                    ...
                    linear_matrices[shell_idx, source_ions, source_ions + 1] = (
                        1.0
                    )
    ...
    def solve_prepared(
        ...
        if np.any(shell_indices < 0):
            raise ValueError(
                "Prepared ion matrices do not contain every shell."
            )
