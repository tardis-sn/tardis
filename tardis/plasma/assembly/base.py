...
def setup_continuum_interactions(
    ...
    continuum_plasma_modules += continuum_interaction_properties
    continuum_plasma_modules += continuum_interaction_inputs

    if config.plasma.continuum_interaction.enable_adiabatic_cooling:
        continuum_plasma_modules += adiabatic_cooling_properties

    if config.plasma.continuum_interaction.enable_two_photon_decay:
        continuum_plasma_modules += two_photon_properties

    transition_probabilities_outputs = [
        plasma_property.transition_probabilities_outputs
        for plasma_property in continuum_plasma_modules
        if issubclass(plasma_property, TransitionProbabilitiesProperty)
    ]
    transition_probabilities_outputs = [
        item for sublist in transition_probabilities_outputs for item in sublist
    ]
    ...
    if (
        ...
        if config.plasma.nlte_ionization_species:
            ...
            for species in nlte_ionization_species:
                if species not in config.plasma.continuum_interaction.species:
                    raise PlasmaConfigError(
    ...
    return continuum_plasma_modules
