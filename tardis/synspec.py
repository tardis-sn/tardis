...
def get_lambda_spec(nu, energy, wave_min, wave_max, samples=50):
    ...
    return wavelength, intens*constants.c/wavelength**2
...
class tardis_result(object):
    def __init__(self,
        ...
        wave, intens = get_lambda_spec(nu_input, np.ones_like(nu_input) * energy_of_packet, wave_min *1e-8, wave_max * 1e-8, samples=samples)
        ...
        wave, intens = get_lambda_spec(nu_reabsorbed, energy_reabsorbed, wave_min *1e-8, wave_max * 1e-8, samples=samples)
