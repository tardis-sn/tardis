...
empty_list = np.array([]).astype(np.float64)
...
@pytest.mark.parametrize(("insert_value", "expected_insert_position"), [
    ...
    (2.5, 8),
    (1.5, 9)])
def test_reverse_binary_search(insert_value, expected_insert_position):
    print(dir(montecarlo))
    insert_position = montecarlo.reverse_binary_search_wrapper(test_line_list, insert_value, 0, len(test_line_list))
    assert insert_position == expected_insert_position

@pytest.mark.parametrize(("insert_value"), [
    (10.5),
    (0.5)])
def test_reverse_binary_search_out_of_bounds(insert_value, capsys):
    with pytest.raises(ValueError):
        insert_position = montecarlo.reverse_binary_search_wrapper(test_line_list, insert_value, 0, len(test_line_list)-1)

#this test acutally fails right now. The reverse_binary_search reads from array not checking whether
#the range (imin,imax) is not empty. Is it fine?
# def test_reverse_binary_search_does_not_fail_on_empty_array():
#     insert_position = montecarlo.reverse_binary_search_wrapper(empty_list, 1.0, 0, len(test_line_list)-1)
#     assert insert_position == 1
