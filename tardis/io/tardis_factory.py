        ...
        def _wrapper(self, *args, **kwargs):
            ...
            if not hasattr(self, attr):
                if not hasattr(self, f'to_{attr}'):
                    logger.warn(f'No attribute {attr} provided, setting {attr}=None')
...
class TARDISFactory(Configuration):
    ...
    @classmethod
    def from_config_dict(cls, config_dict, validate=True, config_dirname="", **kwargs):
        new_config = super().from_config_dict(config_dict, validate=True, config_dirname="")
    ...
    def to_power_law(self):
        ...
        time_0 = self.model.structure.density.get("time_0", self.model.structure.density.time_explosion)
    ...
    @requires('atom_data') # This method and associated property is poorly worded
    ...
    def to_plasma_state(self):
        ...
        self.plasma_state = assemble_plasma(
            self, self.model_state, self.radiation_field, atom_data=self.atomic_data)
        return self.plasma_state

    @requires('model_state', 'radiation_field', 'transport_state', 'plasma_state')
    ...
    def to_model_state(self):
        ...
        self.model_state = ModelState(self.composition, self.geometry, self.time_explosion)
        return self.model_state
        

    def to_virtual_packet_logging(self):
        self.virtual_packet_logging = self.spectrum.virtual.virtual_packet_logging
    ...
    def to_composition(self):
        ...
        if abundances_section.type == "uniform": # The only supported scheme
            elemental_mass_fraction, isotope_abundance = read_uniform_abundances(
                abundances_section, self.no_of_shells
            )
        ...
        elemental_mass_fraction = elemental_mass_fraction[elemental_mass_fraction.sum(axis=1) > 0]

        norm_factor = elemental_mass_fraction.sum(axis=0) + isotope_abundance.sum(axis=0)
    ...
    def to_radiation_field(self):
        ...
        opacities = None
        self.radiation_field = MonteCarloRadiationFieldState(t_radiative, dilution_factor, opacities, self.source_function)
    ...
    def to_source_function(self):
        ...
        if not hasattr(self, 'packet_source') is None:
            if not self.montecarlo.enable_full_relativity:
                self.packet_source = BlackBodySimpleSource(
        ...
        self.source_function = PhotosphericBlackBody1D(v_inner_boundary, t_inner, no_of_packets, self.packet_source)
