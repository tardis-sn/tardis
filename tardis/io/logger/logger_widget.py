...
class PanelWidgetLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if isinstance(record.msg, pd.DataFrame):
            ...
            html_output = (
                f'<pre style="white-space: pre; margin: 0;">{df_str}</pre>'
            )
        else:
    ...
    def _flush_buffer(self, output_key):
        ...
        if (
            not self.log_buffers[output_key]
            or output_key not in self.log_columns
        ):
            return
        ...
        batch_html = "".join(
            [
                f"<div style='margin: 2px 0; padding: 2px 0;'>{log}</div>"
                for log in self.log_buffers[output_key]
            ]
        )
        current_content = (
            getattr(level_column, "_log_content", "")
            or level_column.object
            or ""
        )
        new_content = (
            current_content + batch_html if current_content else batch_html
        )
        ...
        if (
            (self.environment == "jupyter" or self.environment == "ssh_jh")
            and output_key in self.display_handles
    ...
    def _adjust_column_height(self, column, content=None):
        ...
        if hasattr(column, "_start_height") and hasattr(column, "_max_height"):
            ...
            if content is None:
                content = (
                    getattr(column, "_log_content", "") or column.object or ""
                )
