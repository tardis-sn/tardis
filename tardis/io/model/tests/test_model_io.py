...
def test_read_stella_model_meta(stella_model_example_file1):
    ...
    np.testing.assert_almost_equal(
        stella_model_example_file1.metadata["inner_boundary_mass"].to(u.g).value,
        5.190242521200000e33,
    )
    np.testing.assert_almost_equal(
        stella_model_example_file1.metadata["total_mass"].to(u.g).value, 2.618867335600000e34
    )
...
def test_read_stella_model_data(stella_model_example_file1):
    ...
    Test reading a STELLA model file
    """
    np.testing.assert_almost_equal(stella_model_example_file1.data.iloc[0,0], 6.006769337200000E+29)
    np.testing.assert_almost_equal(stella_model_example_file1.data.iloc[-1,-1], 2.123224906916000E+04)
