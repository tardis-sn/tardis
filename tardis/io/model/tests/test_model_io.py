...
def test_read_stella_model():
    ...
    np.testing.assert_almost_equal(model.metadata["t_max"].to(u.day).value, 50.0)
    np.testing.assert_almost_equal(model.metadata["inner_boundary_mass"].to(u.g).value, 5.190242521200000E+33)
    np.testing.assert_almost_equal(model.metadata["total_mass"].to(u.g).value, 2.618867335600000E+34)