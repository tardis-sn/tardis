...
def test_read_csv_composition_default_skip_rows(csv_composition_fname):
    ...
    index, abundances, isotope_abundance = read_csv_isotope_mass_fractions(
        csv_composition_fname, skip_rows=None
    )
...
def test_read_csv_composition_with_skip_rows(csv_composition_fname):
    ...
    assert np.isclose(abundances.loc[7, 8], 0.5, atol=1.0e-12)
    assert np.isclose(abundances.loc[13, 5], 0.8, atol=1.0e-12)
    assert np.isclose(abundances.loc[15, 1], 0.1, atol=1.0e-12)
    assert np.isclose(isotope_abundance.loc[(28, 56), 1], 0.4, atol=1.0e-12)
    assert np.isclose(isotope_abundance.loc[(28, 58), 3], 0.7, atol=1.0e-12)
