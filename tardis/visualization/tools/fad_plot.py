...
class FADPlotter:
    ...
    def from_simulation(cls, sim):
        ...
        data.index = sim.model.velocity[1:]/100000 # inplace the columns with velocity of shell outer boundary
        data.columns = data.columns.map(atomic_number2element_symbol) # map atomic number to element symbol
    ...
    def generate_plot_mpl(self):
        ...
        self.data.plot(figsize=(15, 5), title='Abundance as a function of velocity', marker='o')
    ...
    def generate_plot_ply(self):
        ...
        for col in self.data.columns:
            fig.add_trace(go.Line(x=self.data.index, y=self.data[col], name=self.data[col].name))
