...
class TestSDECPlotter:
    ...
    def test_generate_plot_mpl(
        ...
        expected = regression_data.sync_hdf_store(
            generate_plot_mpl_hdf, update_fname=False
        )
    ...
    def test_generate_plot_ply(
        ...
        regression_data.fname = regression_file
        expected = regression_data.sync_hdf_store(
            generate_plot_plotly_hdf, update_fname=False
        )
    ...
    def test_from_workflow_vs_from_simulation_data_consistency(
        ...
        np.testing.assert_allclose(
            plotter.t_inner.value,
            plotter_from_workflow.t_inner.value,
            atol=0,
            rtol=RELATIVE_TOLERANCE_SDEC,
        )
        np.testing.assert_allclose(
            plotter.r_inner.value,
            plotter_from_workflow.r_inner.value,
            atol=0,
            rtol=RELATIVE_TOLERANCE_SDEC,
        )
        np.testing.assert_allclose(
        ...
        for mode in ["real", "virtual"]:
            ...
            assert (
                plotter_from_workflow.packet_data[mode]["packets_df"]
                is not None
            )
    ...
    def test_calculate_plotting_data_workflow_vs_regression(
        self, plotter_calculate_plotting_data_from_workflow, regression_data
    ):
        param_idx = plotter_calculate_plotting_data_from_workflow._param_idx
        # Reuses the reference data generated by test_calculate_plotting_data,
        # since both objects produce the same plotting data for the same config.
        regression_data.fname = f"test_calculate_plotting_data__plotter_calculate_plotting_data{param_idx}__.h5"
        if regression_data.enable_generate_reference:
            pytest.skip(
                "Reference data is generated by test_calculate_plotting_data"
            )

        group = "plot_data_hdf/"
        with pd.HDFStore(regression_data.fpath, mode="r") as expected:
            for attribute_type, attribute_name in self.plotting_data_attributes:
                plot_object = getattr(
                ...
                if attribute_type == "attributes_np":
                    ...
                    np.testing.assert_allclose(
                        plot_object,
                        expected.get(group + attribute_name).values.flatten(),
                        atol=0,
                        rtol=RELATIVE_TOLERANCE_SDEC,
                    )
    ...
    def test_generate_plot_mpl_workflow_vs_regression(
        self, plotter_generate_plot_mpl_from_workflow, regression_data
    ):
        _, plotter = plotter_generate_plot_mpl_from_workflow
        param_idx = plotter._param_idx
        # Reuses the reference data generated by test_generate_plot_ply,
        # since both objects produce the same plot for the same config.
        regression_data.fname = (
            f"test_generate_plot_mpl__plotter_generate_plot_ply{param_idx}__.h5"
        )
        if regression_data.enable_generate_reference:
            pytest.skip("Reference data is generated by test_generate_plot_ply")

        with pd.HDFStore(regression_data.fpath, mode="r") as expected:
            expected_species = expected.get("plot_data_hdf/_species_name")
            expected_colors = expected.get("plot_data_hdf/_color_list")
    ...
    def test_mpl_image_workflow(
        self, plotter_generate_plot_mpl_from_workflow, tmp_path, regression_data
    ):
        fig, plotter = plotter_generate_plot_mpl_from_workflow
        param_idx = plotter._param_idx
        # Reuses the reference image generated by test_mpl_image, since both
        # objects produce the same plot for the same config.
        regression_data.fname = (
            f"test_mpl_image__plotter_generate_plot_mpl{param_idx}__.png"
        )
        if regression_data.enable_generate_reference:
            pytest.skip("Reference data is generated by test_mpl_image")

        actual_image_path = tmp_path / regression_data.fname
        fig.figure.savefig(actual_image_path)
        compare_images(
            str(regression_data.fpath), str(actual_image_path), tol=1e-3
        )
    ...
    def test_parse_species_list_workflow(
        self, plotter_from_workflow, attribute, regression_data
    ):
        # Parse species list on workflow plotter
        plotter_from_workflow._parse_species_list(self.species_list[0])
        # Reuses the reference data generated by test_parse_species_list,
        # since both objects produce the same parsed species list for the same config.
        regression_data.fname = f"test_parse_species_list__{attribute}__.npy"
        if regression_data.enable_generate_reference:
            pytest.skip(
                "Reference data is generated by test_parse_species_list"
            )

        actual_data = getattr(plotter_from_workflow, attribute)
        expected_data = np.load(regression_data.fpath)
