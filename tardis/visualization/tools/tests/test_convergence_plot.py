...
def fetch_luminosity_data(convergence_plots):
    """Prepare data for luminosity plot."""
...
def test_build(convergence_plots):
    ...
    assert type(convergence_plots.luminosity_plot) == go.FigureWidget

    # check number of traces
    assert len(convergence_plots.luminosity_plot.data) == 5
    assert len(convergence_plots.plasma_plot.data) == 2


@pytest.mark.usefixtures("fetch_luminosity_data")
def test_update_luminosity_plot(convergence_plots):
    """Test the number of traces and length of x and y values."""
    n_iterations = convergence_plots.iterations
    convergence_plots.update_luminosity_plot()
    ...
    for index in range(0, 5):
        ...
        assert (
            len(convergence_plots.luminosity_plot.data[index].x) == n_iterations
        )
        assert (
            len(convergence_plots.luminosity_plot.data[index].y) == n_iterations
        )

    # check range of x axes
    for axis in ["xaxis", "xaxis2", "xaxis3"]:
        convergence_plots.luminosity_plot["layout"][axis]["range"] = [
            0,
...
def test_override_plot_parameters(convergence_plots):
    ...
    convergence_plots.override_plot_parameters(
        convergence_plots.luminosity_plot, parameters=parameters
    )
    ...
    for trace_index in range(5):
        assert (
            convergence_plots.luminosity_plot.data[trace_index].line.dash
            == "dot"
        )
        assert (
            convergence_plots.luminosity_plot.data[trace_index].mode
            == "lines+markers"
        )
    # checking layout
    assert (
        convergence_plots.luminosity_plot["layout"]["xaxis2"]["showgrid"]
        == False
    )
