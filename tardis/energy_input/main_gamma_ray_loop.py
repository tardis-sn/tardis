...
def run_gamma_ray_loop(
    ...
    # escaped energy in ergs/s/Hz
    escape_energy = pd.DataFrame(
        data=energy_out, columns=times[:-1], index=energy_bins
    )
    # deposited energy in ergs/s
    deposited_energy = (
        pd.DataFrame(data=energy_deposited, columns=times[:-1]) / dt_array
    )
    #
    positron_energy = (
        pd.DataFrame(data=positron_energy, columns=times[:-1]) / dt_array
    )
    total_positron_energy = positron_energy.sum().sum()
    logger.info(f"Total positron energy is {total_positron_energy}")
