...
class OpacityState:
    ...
    def to_numba(
        ...
        """
        Initialize the OpacityStateNumba object and copy over the data from OpacityState class
        ...
        """
        electron_densities = cls.electron_density.values
        t_electrons = cls.t_electrons
        line_list_nu = cls.line_list_nu.values

        tau_sobolev = np.ascontiguousarray(cls.tau_sobolev, dtype=np.float64)

        if line_interaction_type == "scatter":
            array_size = 1
            transition_probabilities = np.zeros((array_size, array_size), dtype=np.float64)
            ...
            macro_block_references = np.asarray(macro_atom_state.macro_block_references)
        ...
        # Continuum processes handling
        if montecarlo_globals.CONTINUUM_PROCESSES_ENABLED:
            bf_threshold_list_nu = cls.continuum_state.bf_threshold_list_nu.values
            p_fb_deactivation = np.ascontiguousarray(
                cls.continuum_state.p_fb_deactivation.values.copy(),
                dtype=np.float64,
            )
            phot_nus = cls.continuum_state.phot_nus
            photo_ion_block_references = cls.continuum_state.photo_ion_block_references
            photo_ion_nu_threshold_mins = cls.continuum_state.photo_ion_nu_threshold_mins.values
            photo_ion_nu_threshold_maxs = cls.continuum_state.photo_ion_nu_threshold_maxs.values
            chi_bf = cls.continuum_state.chi_bf.values
            x_sect = cls.continuum_state.x_sect.values
            phot_nus = phot_nus.values
            ff_opacity_factor = (cls.continuum_state.ff_cooling_factor / np.sqrt(t_electrons)).astype(np.float64)
            emissivities = cls.continuum_state.emissivities.values
            photo_ion_activation_idx = cls.continuum_state.photo_ion_activation_idx.values
            k_packet_idx = np.int64(cls.continuum_state.k_packet_idx)
        else:
