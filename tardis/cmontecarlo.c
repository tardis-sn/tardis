					...
					storage_model_t * storage)
{
	...
	if (rpacket_get_recently_crossed_boundary(packet) == 1) {
		...
		if (check < 0.0) {
			...
			d_inner =
			    mu < 0.0 ? -r * mu - sqrt(check) : MISS_DISTANCE;
		}
	}
	if (d_inner < d_outer) {
		rpacket_set_next_shell_id(packet, -1);
			  ...
			  double distance)
{
	...
	if (distance > 0.0) {
		...
		if (rpacket_get_virtual_packet(packet) <= 0) {
			comov_energy =
			    rpacket_get_energy(packet) * doppler_factor;
			      ...
			      int64_t virtual_mode)
{
	...
	if (virtual_mode == 0) {
		...
		for (i = 0; i < rpacket_get_virtual_packet_flag(packet); i++) {
			memcpy((void *)&virt_packet, (void *)packet,
			       sizeof(rpacket_t));
			...
			mu_bin =
			    (1.0 -
			     mu_min) / rpacket_get_virtual_packet_flag(packet);
			virt_packet.mu =
			    mu_min + (i + rk_double(&mt_state)) * mu_bin;
			switch (virtual_mode) {
			case -2:
				weight =
				    1.0 /
			...
			virt_packet.nu =
			    rpacket_get_nu(packet) * doppler_factor_ratio;
			reabsorbed =
			    montecarlo_one_packet_loop(storage, &virt_packet,
						       1);
...
void move_packet_across_shell_boundary(rpacket_t * packet,
				       storage_model_t * storage,
				       double distance)
{
	...
	if (rpacket_get_virtual_packet(packet) > 0) {
		double delta_tau_event = distance *
		    storage->
		    electron_densities[rpacket_get_current_shell_id(packet)] *
	...
	if ((rpacket_get_current_shell_id(packet) < storage->no_of_shells - 1
		...
		inverse_doppler_factor =
		    1.0 / rpacket_doppler_factor(packet, storage);
...
void montecarlo_thomson_scatter(rpacket_t * packet, storage_model_t * storage,
				double distance)
{
	double comov_energy, doppler_factor, comov_nu, inverse_doppler_factor;
...
void montecarlo_line_scatter(rpacket_t * packet, storage_model_t * storage,
			     double distance)
{
	...
	if (rpacket_get_virtual_packet(packet) == 0) {
		j_blue_idx =
		    rpacket_get_current_shell_id(packet) *
		    storage->line_lists_j_blues_nd +
		    rpacket_get_next_line_id(packet);
		increment_j_blue_estimator(packet, storage, distance,
					   j_blue_idx);
	}
	tau_line =
	    storage->
...
INLINE void montecarlo_compute_distances(rpacket_t * packet,
					 storage_model_t * storage)
{
