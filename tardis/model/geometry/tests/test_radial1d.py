...
def test_vb_indices(radial_1d_geometry):
    # Testing if the indices returned are correct when inner and outer
    # boundary are on the innermost and outermost shell

    radial_1d_geometry.v_inner_boundary = (
        radial_1d_geometry.v_inner[0]
    )
    radial_1d_geometry.v_outer_boundary = (
        radial_1d_geometry.v_outer[-1]
    )
    assert radial_1d_geometry.v_inner_boundary_idx == 0
    assert radial_1d_geometry.v_outer_boundary_idx == len(
        radial_1d_geometry.v_inner
    )
    vib_index = radial_1d_geometry.v_inner_boundary_idx
    vob_index = radial_1d_geometry.v_outer_boundary_idx
    assert np.all(
        radial_1d_geometry.v_inner[vib_index:vob_index]
        == radial_1d_geometry.v_inner
    )
    ...
    radial_1d_geometry.v_inner_boundary = (
        radial_1d_geometry.v_inner[0] + EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_inner_boundary_idx == 0
    radial_1d_geometry.v_inner_boundary = (
        radial_1d_geometry.v_inner[0] - EPSILON_VELOCITY_SHIFT
    ...
    radial_1d_geometry.v_inner_boundary = (
        radial_1d_geometry.v_inner[1] - EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_inner_boundary_idx == 0
    radial_1d_geometry.v_inner_boundary = (
        radial_1d_geometry.v_inner[1] + EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_inner_boundary_idx == 1

    # pivoting around the outer boundary of the simulation
    radial_1d_geometry.v_outer_boundary = (
        radial_1d_geometry.v_outer[-1] + EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_outer_boundary_idx == 12
    radial_1d_geometry.v_outer_boundary = (
        radial_1d_geometry.v_outer[-1] - EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_outer_boundary_idx == 12
    ...
    radial_1d_geometry.v_outer_boundary = (
        radial_1d_geometry.v_outer[-2] + EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_outer_boundary_idx == 12
    radial_1d_geometry.v_outer_boundary = (
        radial_1d_geometry.v_outer[-2] - EPSILON_VELOCITY_SHIFT
    )
    assert radial_1d_geometry.v_outer_boundary_idx == 11


def test_numba_nonhomologous_velocity(radial_1d_geometry):
    numba_geometry = radial_1d_geometry.to_numba()
...
def test_velocity_boundary(radial_1d_geometry):
    # testing the active cell boundaries when setting the boundaries

    radial_1d_geometry.v_inner_boundary = 7999 * u.km / u.s
    npt.assert_almost_equal(
        radial_1d_geometry.v_inner_active[0].value, 7999
    )
    assert len(radial_1d_geometry.v_inner_active) == len(
        radial_1d_geometry.v_inner
    )

    radial_1d_geometry.v_inner_boundary = 8001 * u.km / u.s
    npt.assert_almost_equal(
        radial_1d_geometry.v_inner_active[0].value, 8001
    )
    assert len(radial_1d_geometry.v_inner_active) == len(
        radial_1d_geometry.v_inner
    )

    radial_1d_geometry.v_inner_boundary = 9001 * u.km / u.s
    npt.assert_almost_equal(
        radial_1d_geometry.v_inner_active[0].value, 9001
    )
    assert len(radial_1d_geometry.v_inner_active) == (
        len(radial_1d_geometry.v_inner) - 1
    )
    radial_1d_geometry.v_inner_boundary = 9000 * u.km / u.s
    npt.assert_almost_equal(
        radial_1d_geometry.v_inner_active[0].value, 9000
    )
    assert len(radial_1d_geometry.v_inner_active) == (
        len(radial_1d_geometry.v_inner) - 1
    )
...
def test_velocity_boundary_updates_radius_boundary(
    ...
    radial_1d_geometry.v_inner_boundary = v_inner_boundary
    radial_1d_geometry.v_outer_boundary = v_outer_boundary

    assert u.isclose(
        radial_1d_geometry.r_inner_active[0],
        (v_inner_boundary * time_explosion).cgs,
    )
    assert u.isclose(
        radial_1d_geometry.r_outer_active[-1],
        (v_outer_boundary * time_explosion).cgs,
    )


def test_v_middle_active_default_boundaries(radial_1d_geometry):
    """Test v_middle_active with default (full range) boundaries"""
    v_middle_active = radial_1d_geometry.v_middle_active

    # Check that v_middle_active has correct length
    assert len(v_middle_active) == len(
        radial_1d_geometry.v_inner_active
    )
    assert len(v_middle_active) == len(
        radial_1d_geometry.v_outer_active
    )
...
def test_v_middle_active_modified_inner_boundary(
    ...
    radial_1d_geometry.v_inner_boundary = 9500 * u.km / u.s
...
def test_v_middle_active_modified_outer_boundary(
    ...
    radial_1d_geometry.v_outer_boundary = 19500 * u.km / u.s
...
def test_v_middle_active_narrow_range(radial_1d_geometry):
    """Test v_middle_active with a narrow range of active shells"""
    # Set boundaries to include only a few shells
    radial_1d_geometry.v_inner_boundary = 11500 * u.km / u.s
    radial_1d_geometry.v_outer_boundary = 14500 * u.km / u.s
