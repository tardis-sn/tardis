...
def test_compare_models(model_config_files):
    """Compare models produced by .from_config and .from_csvy to check that velocities, densities and abundances(pre and post decay) are the same"""
    ...
    npt.assert_array_almost_equal(csvy_model.abundance.to_numpy(),config_model.abundance.to_numpy())
...
def test_read_csvy_abundances(csvy_model_abundances):
    """test if model read abundances and isotope abundances and constructs dataframes appropiatly before applying decay"""
    ...
    npt.assert_array_almost_equal(input_abundance.to_numpy(),csvy_model_abundances.raw_abundance.to_numpy())
    ...
    npt.assert_array_almost_equal(input_isotopes.to_numpy(),csvy_model_abundances.raw_isotope_abundance.to_numpy())
    


def test_csvy_model_decay(csvy_model_abundances):
    """Compare model decay of abundance and hand decayed abundances:
