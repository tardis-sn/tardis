...
class GenericModel():
    ...
    def __init__(self, *properties, time= None):
        ...
        try:    
            ...
            print("No time for the model was provided, time of properties will be used")
            max_property_time = 0 * u.s
        self.names = []
        for tproperty in properties:
            setattr(self, tproperty.name, tproperty)
            self.names.append(tproperty.name)    
            max_property_time = u.Quantity(max(max_property_time, tproperty.time))
        ...
        try:
            ...
            raise ValueError(
                ...
                'by looking at other quantities, the earliest time you could set is'+
                '{max_property_time}')
...
class BaseProperty():
    ...
    def check_number_of_cells(self, number_of_cells):
        if self.number_of_cells != number_of_cells:
            raise ValueError(f"{self.name} number of cells is inconsistent with that of the model.\
                             It should be {number_of_cells} but is {self.number_of_cells}")
...
class Velocity(BaseProperty):
    ...
    def __init__(self, velocity_inner, velocity_outer, 
        ...
        self.dataframe_column_names = {'inner': 'velocity_inner',
    ...
    @property
    def volume(self):
        volume = (4./3 * np.pi * (self.outer**3 - self.inner**3) * self.time**3)
        return volume.cgs
...
class Density(BaseProperty):
    ...
    @classmethod
    def from_exponential(cls, rho_0, velocity_object, exponent, electron_density, time_0):
        density = rho_0 * \
            np.exp(-(velocity_object.middle / velocity_object.inner[0]))
    ...
    @u.quantity_input
    def cgs_electron(self, electron_density: 1. / u.cm**3):
...
class Abundances(BaseProperty):
    ...
    def get_isotope_index(self, atomic_number_index, mass_number_index):
        ...
        for atomic_number, mass_number in zip(atomic_number_index, mass_number_index):
            names_index.append('{0:s}{1:d}'.format(
...
class RadiationField(BaseProperty):
    ...
    def __init__(self, radiative_temperature= None, dilution_factor=None, time_0=0 * u.d):
        ...
        self.number_of_cells = self.set_number_of_cells(radiative_temperature, dilution_factor)
        self.dataframe_column_names = {'radiative_temperature': 'radiative_temperature',
    ...
    def set_number_of_cells(self,radiative_temperature, dilution_factor):
        if radiative_temperature and dilution_factor is not None:
            if len(radiative_temperature) == len(dilution_factor):      
                ...
                raise ValueError('radiative_temperature and ' + 
