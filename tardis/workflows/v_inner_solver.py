...
class InnerVelocitySolverWorkflow(StandardTARDISWorkflow):
    ...
    def __init__(
        ...
        active_shell_range = (
            self.simulation_state.geometry.v_inner_boundary_idx,
            self.simulation_state.geometry.v_outer_boundary_idx,
        )
        self.simulation_state.geometry.v_inner_boundary = initial_v_inner
        if active_shell_range != (
            self.simulation_state.geometry.v_inner_boundary_idx,
            self.simulation_state.geometry.v_outer_boundary_idx,
        ):
            self._assemble_active_plasma()
        self.opacity_states = self.solve_opacity()
    ...
    def _assemble_active_plasma(self) -> None:
        """Assembles a plasma with active geometry"""
    ...
    def store_plasma_state(
        ...
        """
        # Recompute integrated tau because the shape of the model can change
        tau_integ = np.log(
            get_tau_integ(
                self.plasma_solver,
    ...
    def estimate_v_inner(self):
        ...
        logger.info(
            "Estimated v_inner_boundary: %s",
            f"{estimated_v_inner.to(u.km / u.s):.2f}",
        )
    ...
    def solve_simulation_state(self, estimated_values):
        ...
        # reassemble the plasma if the geometry has changed
        if active_shell_range != (
            self.simulation_state.geometry.v_inner_boundary_idx,
            self.simulation_state.geometry.v_outer_boundary_idx,
        ):
    ...
    def run(self):
        ...
        # Recompute integrated optical depth a last time
        self.tau_integ = np.log(
            get_tau_integ(
                self.plasma_solver,
                self.opacity_states["opacity_state"],
