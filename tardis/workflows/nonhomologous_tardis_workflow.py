...
class NonhomologousTARDISWorkflow(WorkflowLogging):
    ...
    def __init__(self, configuration: Configuration, csvy: bool = False):
        ...
        # connor-mcclellan: the PlasmaSolverFactory class has been duplicated for nonhomology, but
        # only the `assemble` method here is actually modified since it passed time_explosion
        # the base plasma class previously took time_explosion only as **kwargs, so it doesn't need a
        # nonhomologous equivalent class
    ...
    def check_convergence(
        ...
        if np.all(convergence_statuses):
            ...
            logger.info(
                f"Iteration converged {self.consecutive_converges_count:d}/{(hold_iterations + 1):d} consecutive "  # noqa: G004
                f"times."
            )
