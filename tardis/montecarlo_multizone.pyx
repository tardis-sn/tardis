...
def montecarlo_radial1d(model, photon_packets):
    """
    Parameters
    ---------
    ...
    cdef np.ndarray[float_type_t, ndim=1] packet_nus = photon_packets.packet_nus
    cdef np.ndarray[float_type_t, ndim=1] packet_mus = photon_packets.packet_mus
    cdef np.ndarray[float_type_t, ndim=1] packet_energies = photon_packets.packet_energies
    ...
    cdef int_type_t no_of_shells = model.no_of_shells
    ...
    #electron density
    cdef np.ndarray[float_type_t, ndim=1] current_electron_density = model.electron_density[0]
    cdef np.ndarray[float_type_t, ndim=1] current_inverse_electron_density = 1 / current_electron_density

    #line lists
    cdef np.ndarray[float_type_t, ndim=1] current_line_list_nu = model.tau_sobolevs[0][:,0]
    cdef np.ndarray[float_type_t, ndim=1] current_tau_sobolev = model.tau_sobolevs[0][:,1]
...
#                logger.info("At packet %d of %d", i, no_of_packets)
    ...
    return output_nus, output_energies
        ...
        while True:
            ...
            #                print "electron scattering happened"
                ...
                doppler_factor = move_packet(&current_r, &current_mu, current_nu, current_energy, d_electron, js, nubars
                    , inverse_time_explosion, current_zone_id)
            ...
            #Line scattering
                ...
                #                    print "line event happened"
                    ...
                    old_doppler_factor = move_packet(&current_r, &current_mu, current_nu, current_energy, d_line, js,
                        nubars, inverse_time_explosion, current_zone_id)
