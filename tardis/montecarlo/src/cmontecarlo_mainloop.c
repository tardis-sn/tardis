			 ...
			 int64_t virtual_packet_flag)
{
	...
	printf("Start MC run with %d threads.\n",openmp_threads);
...
#pragma single
		{
			...
			big_packet =
			    (rpacket_t *) safe_malloc(sizeof(rpacket_t) *
						      num_threads);

			big_spectrum_virt_nu_todo_index =
			    (int64_t *) safe_malloc(sizeof(int64_t) *
						    num_threads * MAX_TODO_LEN);
			big_spectrum_virt_nu_todo_value =
			    (double *)safe_malloc(sizeof(int64_t) *
						  num_threads * MAX_TODO_LEN);

			big_line_lists_j_blues_todo_index =
			    (int64_t *) safe_malloc(sizeof(int64_t) *
						    num_threads * MAX_TODO_LEN);
			big_line_lists_j_blues_todo_value =
			    (double *)safe_malloc(sizeof(int64_t) *
						  num_threads * MAX_TODO_LEN);
		       ...
		       int64_t virtual_packet_flag)
{
	...
	big_spectrum_virt_nu_todo_index =
	    (int64_t *) safe_malloc(sizeof(int64_t) * MAX_TODO_LEN);
	big_spectrum_virt_nu_todo_value =
	    (double *)safe_malloc(sizeof(int64_t) * MAX_TODO_LEN);
...
#pragma omp atomic
			storage->spectrum_virt_nu[ii] +=
			    packet->spectrum_virt_nu_todo_value[i];
		}
	}
...
#pragma omp atomic
			storage->line_lists_j_blues[ii] +=
			    packet->line_lists_j_blues_todo_value[i];
		}
	}
}

void
assign_packet_todo(rpacket_t * packet, int64_t thread_num,
		   ...
		   double *big_line_lists_j_blues_todo_value)
{
	...
	packet->spectrum_virt_nu_todo_index =
	    (int64_t *) big_spectrum_virt_nu_todo_index +
	    (thread_num * MAX_TODO_LEN);
	packet->spectrum_virt_nu_todo_value =
	    (double *)big_spectrum_virt_nu_todo_value +
	    (thread_num * MAX_TODO_LEN);
	packet->line_lists_j_blues_todo_len = 0;
	packet->line_lists_j_blues_todo_index =
	    (int64_t *) big_line_lists_j_blues_todo_index +
	    (thread_num * MAX_TODO_LEN);
	packet->line_lists_j_blues_todo_value =
	    (double *)big_line_lists_j_blues_todo_value +
	    (thread_num * MAX_TODO_LEN);
}
