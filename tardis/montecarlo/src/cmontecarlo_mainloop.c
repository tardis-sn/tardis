		       ...
		       int64_t virtual_packet_flag)
{
  if (openmp_threads == 0)
    {
      montecarlo_serial_loop (storage, openmp_threads, virtual_packet_flag);
    }
  else
    {
      montecarlo_parallel_loop (storage, openmp_threads, virtual_packet_flag);
    }
}
...
#pragma omp atomic
  ...
  for (int i = 0; i < lljb_len; i++)
    {
#pragma omp atomic
      storage->line_lists_j_blues_[i] += packet->line_lists_j_blues_[i];
    }
}
