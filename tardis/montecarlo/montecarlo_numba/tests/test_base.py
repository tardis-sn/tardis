...
def test_montecarlo_main_loop(
    ...
    expected_nu_bar_estimator = pd.read_hdf(C_fname, key="/simulation/runner/nu_bar_estimator").values
    expected_j_estimator = pd.read_hdf(C_fname, key="/simulation/runner/j_estimator").values
    ...
    for i in range(len(packet_collection.packets_input_nu)):
        #Generate packet
        packet = r_packet.RPacket(numba_model.r_inner[0],
    ...
    #Compare
    npt.assert_allclose(actual_nu_bar_estimator, expected_nu_bar_estimator, rtol=1e-13)
