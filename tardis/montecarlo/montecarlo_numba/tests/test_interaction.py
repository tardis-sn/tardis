...
def test_line_scatter(line_interaction_type, packet, verysimple_numba_model, verysimple_numba_plasma):
    init_mu = packet.mu
    init_nu = packet.nu
    init_energy = packet.energy
    ...
    interaction.line_scatter(packet, time_explosion, line_interaction_type, verysimple_numba_plasma)
...
@pytest.mark.parametrize(
    ...
    [({'mu': 0.8599443103322428, 'emission_line_id': 1000, 'energy': 0.9114437898710559, 'nu': 0.0},
    {'mu': 0.8599443103322428, 'energy': 0.9114437898710559}),
    ({'mu': -0.6975116557422458, 'emission_line_id': 2000, 'energy': 0.8803098648913266},
    {'mu': -0.6975116557422458, 'energy': 0.8803098648913266}),
    ({'mu': -0.7115661419975774, 'emission_line_id': 0, 'energy': 0.8800385929341252},
    {'mu': -0.7115661419975774, 'energy': 0.8800385929341252})]
)
def test_line_emission(packet, verysimple_numba_model, verysimple_numba_plasma, test_packet, expected):
    ...
    interaction.line_emission(packet, emission_line_id, time_explosion, verysimple_numba_plasma)
