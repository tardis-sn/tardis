...
def test_levels_property(excitation_energy):
    assert np.isclose(excitation_energy.iloc[2].iloc[0].iloc[1], 3.17545416e-11)


def test_lines_property(lines):
    assert np.isclose(lines.iloc[564954]["wavelength"], 10833.307)
...
def test_atomic_mass_property(atomic_mass):
    assert np.isclose(atomic_mass.iloc[2], 6.6464755216973998e-24)


def test_ionization_data_property(ionization_data):
    assert np.isclose(float(ionization_data.iloc[2].iloc[1]), 3.9393336e-11)


def test_zeta_data_property(zeta_data):
    assert np.isclose(zeta_data.iloc[2].iloc[1][2000], 0.4012)
