...
default_photoionization_h5_path = os.path.join(os.path.dirname(__file__),
                                               'data',
                                               'photoionization_data_H30_He.h5')
...
class BaseContinuumData(object):
    def __init__(self, atom_data, photo_dat_fname=None,
        ...
        self.multi_index_nu_sorted = self.continuum_data.sort_values('nu', ascending=False).index
...
class MCDataMixin(object):
    def _set_montecarlo_data(self):
        nu_sorted_continuum_data = self.continuum_data.sort_values('nu', ascending=False)
        ...
        self.bf_emissivities_block_references_old = np.hstack([np.array([0]),
            self.photoionization_data['nu'].groupby(level=[0,1,2]).count().cumsum().values])
        block_ref = self.photoionization_data['nu'].groupby(level=[0,1,2]).count()
...
class PhotoionizationData(object):
    ...
    def from_hdf5(cls, fname, atom_data):
        ...
        selected_atoms = [species[0] for species in atom_data.selected_continuum_species]
        # TODO: Fixme! Have to also use ionization stage here
        photoionization_data = photoionization_data.loc[pd.IndexSlice[selected_atoms,:,:], :]
