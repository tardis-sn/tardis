...
class BaseContinuum(object):
    ...
    def _diagonalize_ma(self):
        ...
        internal_mask = np.logical_or(
                trans_prob.transition_type == 1,
                trans_prob.transition_type == 0
        )
        ...
        cont_jump_prob = self.transition_probabilities.dataframe.query(
            'transition_type == 2')
        cont_ma_jump_prob = self.recombination_transition_probabilities.dataframe.query(
            'transition_type == 0')
        ...
        for shell in range(no_shells):
            ...
            int_jump_matrix = int_jump_prob.loc[index, shell].fillna(0).values.reshape(
                (no_lvls, no_lvls))
            ...
            cj_dest_idx = cont_jump_prob.index.get_level_values(1).values + no_lvls
            ...
            c_ma_source_idx = cont_jump_prob.index.get_level_values(1).values + no_lvls
        ...
        trans_prob = self.transition_probabilities._normalize_transition_probabilities(
        ...
        recomb_prob = self.recombination_transition_probabilities.dataframe.query(
            'transition_type==-2 | transition_type==-3')
        recomb_prob = self.transition_probabilities._normalize_transition_probabilities(
