...
class PhiSahaElectrons(PhiSahaLTE):
    ...
    @staticmethod
    def calculate(g_electron_Te, beta_electron, lte_partition_function_Te, ionization_data):
...
class RadiationFieldCorrection(ProcessingPlasmaProperty):
    ...
    def __init__(self, plasma_parent=None, departure_coefficient=None,
        chi_0_species=(1,1), delta_treatment=None):
...
class LTEIonNumberDensity(IonNumberDensity):
    ...
    def calculate(self, phi_Te, lte_partition_function_Te, number_density, electron_densities):
        return self.calculate_with_n_electron(
            phi_Te, lte_partition_function_Te, number_density, electron_densities)
...
class NLTEIonNumberDensity(ProcessingPlasmaProperty):
    ...
    def calculate(self, phi, alpha_stim, alpha_sp, gamma, coll_ion_coeff, coll_recomb_coeff,
                  number_density, level_boltzmann_factor):
        ...
        if gamma is not None:
            ...
            coll_ion_tot = self._calculate_tot_ion_rate(coll_ion_coeff, level_pop_fractions, partition_function.index)
            coll_recomb_tot = self._calculate_coll_recomb_tot(coll_recomb_coeff, partition_function.index)
        for i in range(partition_function.shape[1]):
            initial_guesses = self._generate_initial_guesses(
                number_density_vectors, number_density_vectors_neutral,
                number_density, shell=i
            )
            for initial_guess in initial_guesses:
                ...
                func, jac = self._get_functions(charge_conservation_vector, number_density_vectors,
                                            number_conservation_index, phi_prep, *rates, shell=i)
                sol = root(fun=func, x0=initial_guess, jac=jac,
                        options={'xtol':1e-12})
                ...
                raise PlasmaIonizationError(
                    'Ionization calculation failed in shell {}.'.format(i))
        ...
        if gamma is not None:
            ...
            if self.plasma_parent.previous_ion_number_density is not None:
                conv = np.fabs((self.plasma_parent.previous_ion_number_density - ion_number_densities) /
    ...
    def _check_number_conservation(ion_number_density, number_density, rtol=1e-5):
        ...
        if not success_number_conservation:
            logger.warn('Number conservation violated:\n{}'.format(rdiff))
    ...
    def _get_functions(self, charge_conservation_vector, number_density_vectors, number_conservation_index, phi_prep,
                       alpha_tot, gamma_tot, coll_ion_tot, coll_recomb_tot, shell):
        ...
        if alpha_tot is not None:
            ...
            coll_recomb_matrix = self._setup_recomb_rate_matrix(coll_recomb_tot[shell].values)
            coll_ion_matrix = self._setup_ion_rate_matrix(coll_ion_tot[shell].values)

            nlte_mask = (alpha_matrix + gamma_matrix + coll_ion_matrix +
                         coll_recomb_matrix) != 0.
        ...
        def setup_rate_matrix(x):
            ...
            if alpha_tot is not None:
                ...
                rate_matrix[nlte_mask] += (alpha_matrix * electron_density +
                                           gamma_matrix)[nlte_mask]
                rate_matrix[nlte_mask] += (coll_ion_matrix * electron_density +
                                           coll_recomb_matrix * electron_density
                                           ** 2.)[nlte_mask]
    ...
    def _get_nlte_species_mask(self, gamma_tot):
        ...
        for species in nlte_species:
            ...
            nlte_species_mask = np.logical_or(nlte_species_mask,
                                              mask1)
        nlte_species_mask = np.hstack(
            [nlte_species_mask, np.zeros(1, dtype=bool)]
        )
        nlte_species_mask = nlte_species_mask.reshape(-1,1)
        nlte_species_mask = np.tile(nlte_species_mask,
                                    len(nlte_species_mask))
    ...
    def _prepare_phi(phi, ion_index):
        ...
        if no_nans:
            logger.warn('Phis contain {} Nans'.format(no_nans))
    ...
    def _get_number_conservation_index(ion_index):
        ...
        index1 = np.concatenate([np.ones(j+1, dtype=int) * i for i,j in zip(sum1, atomic_number)])
    ...
    def _generate_initial_guesses(
            ...
            shell):
        if self.plasma_parent.previous_ion_number_density is not None:
            initial_ion_density = self.plasma_parent.previous_ion_number_density[shell].values
            initial_electron_density = self.plasma_parent.previous_electron_densities[shell]
        ...
        singly_ionized_index = pd.MultiIndex.from_product([number_density.index, [1]]).values
        initial_ion_density.loc[singly_ionized_index] = number_density[shell].values
