...
from tardis.plasma.exceptions import PlasmaConfigError, PlasmaNLTEExcitationError
...
class LevelBoltzmannFactorNLTE(ProcessingPlasmaProperty):
    ...
    def _main_nlte_calculation(self, atomic_data, nlte_data,
        ...
        for species in self.plasma_parent.nlte_species:
            ...
            rates_matrix, x, lines_idx, r_lu_matrix, r_ul_matrix = \
                self._setup_bb_rates(species, atomic_data, nlte_data, t_electrons,
                                    j_blues, beta_sobolevs, previous_electron_densities)
    ...
    def _calculate_with_continuum(self, t_electrons, lines, atomic_data, nlte_data,
                                  general_level_boltzmann_factor, j_blues,
                                  previous_beta_sobolev, lte_j_blues, previous_electron_densities, gamma,
                                  previous_ion_number_density, alpha_stim, alpha_sp, coll_exc_coeff, coll_deexc_coeff,
                                  coll_ion_coeff, coll_recomb_coeff):
        """
        Full NLTE calculation without approximations.
        """
        if previous_beta_sobolev is None:
        ...
        if len(j_blues)==0:
            ...
            j_blues = pd.DataFrame(j_blues, index=lines.index, columns =
                range(len(t_electrons)))
        general_level_boltzmann_factor, ion_ratio = self._main_nlte_cont_calculation(
            atomic_data, nlte_data, t_electrons, j_blues,
            beta_sobolevs, general_level_boltzmann_factor,
            previous_electron_densities, gamma, previous_ion_number_density, alpha_stim, alpha_sp,
            coll_exc_coeff, coll_deexc_coeff, coll_ion_coeff, coll_recomb_coeff)
        return general_level_boltzmann_factor, ion_ratio

    def _main_nlte_cont_calculation(self, atomic_data, nlte_data,
        t_electrons, j_blues, beta_sobolevs, general_level_boltzmann_factor,
        previous_electron_densities, gamma, previous_ion_number_density, alpha_stim, alpha_sp,
        coll_exc_coeff, coll_deexc_coeff, coll_ion_coeff, coll_recomb_coeff):
        """
        ...
        for species in self.plasma_parent.nlte_species:
            #logger.info('Calculating rates for species %s', species)
            rates_matrix, x, lines_idx, r_lu_matrix, r_ul_matrix, r_lu_index, r_ul_index = \
                self._setup_bb_rates(species, atomic_data, nlte_data, t_electrons,
                                     j_blues, beta_sobolevs, previous_electron_densities,
                                     coll_exc_coeff, coll_deexc_coeff)


            no_of_levels = atomic_data.levels.energy.ix[species].count()
            ...
            for i in xrange(len(t_electrons)):
                if previous_ion_number_density is not None and gamma is not None:
                    ...
                    rates_matrix[:, :, i] -= np.diag(gamma1[i].values +
                                                     coll_ion)

                    gamma_vec = gamma1[i].values + coll_ion
                    next_ion_index = (species[0], species[1] + 1)
                    next_ion_density = previous_ion_number_density.loc[next_ion_index, i]
                    next_ion_density /= previous_ion_number_density.loc[species, i]
                    ...
                    collision_rates = self._setup_collision_rate_matrix(
                        coll_exc_coeff_species, coll_deexc_coeff_species, no_of_levels, i, n_e
                    )
                    ...
                    remaining_rates = -np.diag(gamma1[i].values + coll_ion) + collision_rates
                    # TODO: passing self is horrible
                    #args = {'self': self, 'x_vec': x, 'remaining_rates': remaining_rates}
                    phis = self.plasma_parent.phi.loc[(species[0],), i].values / n_e

                    args = (self, x, remaining_rates, lines_idx, r_lu_matrix[:,:,i], r_ul_matrix[:,:,i],
                            r_lu_index, r_ul_index,
                            self.plasma_parent.number_density.loc[species[0],i],
                            gamma_vec, species, phis)

                    initial = self.plasma_parent.level_number_density[i].ix[species].values
                    initial /= initial.sum()
                    ...
                    if (res.x[:-1] < 0).sum():
                        ...
                        res = least_squares(self._rate_equations,
                            x0=initial, args=args, bounds=(0., ubound))

                        if (res.x[:-1] < 0).sum():
                            raise PlasmaNLTEExcitationError('level_boltzmann_factor is negative - aborting')
    ...
    @staticmethod
    def _rate_equations(x, self, x_vec, remaining_rates, lines_idx, r_lu_matrix,
            r_ul_matrix, r_lu_index, r_ul_index, number_density,
            gamma_vec, species, phis):
        ion_numbers = self._get_ion_numbers(species, phis,
                                            number_density, phi_nlte=x[-1])
        ...
        radiative_rates = self._setup_radiative_rates(beta_sobolev, lines_idx, r_ul_matrix.copy(), r_lu_matrix.copy(), r_lu_index, r_ul_index)
        ...
        #gamma_vec[0] = 0.0
        rates_matrix = np.append(rates_matrix, [np.hstack([gamma_vec, x_tot])], 0)
    ...
    def _setup_radiative_rates(self, beta_sobolev, lines_idx, r_ul_matrix, r_lu_matrix, r_lu_index, r_ul_index):
        #number_of_levels = 20
        ...
        for i in xrange(r_ul_matrix.shape[0]):
    ...
    def _caculate_beta_sobolevs(self, level_number_density):
        ...
        tau_sobolev_calc = self.plasma_parent.plasma_properties_dict['TauSobolev'].calculate
        #beta_sobolev_calc = self.plasma_parent.plasma_properties_dict['BetaSobolev'].calculate
        stim_emission_factor_calc = self.plasma_parent.plasma_properties_dict['StimulatedEmissionFactor'].calculate

        stim_emission_factor = stim_emission_factor_calc(pl.g, level_number_density, pl.lines_lower_level_index,
                  pl.lines_upper_level_index, pl.metastability, pl.lines)

        tau_sobolev = tau_sobolev_calc(pl.lines, level_number_density, pl.lines_lower_level_index,
                  pl.time_explosion, stim_emission_factor, pl.j_blues,
                  pl.f_lu, pl.wavelength_cm)

        #import ipdb; ipdb.set_trace()
        beta_sobolev = np.zeros_like(tau_sobolev.values)

        macro_atom.calculate_beta_sobolev(tau_sobolev.values.ravel(), beta_sobolev.ravel())

        return beta_sobolev


    @staticmethod
    def _setup_bb_rates(species, atomic_data, nlte_data, t_electrons, j_blues, beta_sobolevs,
                        previous_electron_densities, coll_exc_coeff=None, coll_deexc_coeff=None):
        ...
        x[0] = 1.0
        #import ipdb; ipdb.set_trace()
        return rates_matrix, x, lines_index, r_lu_matrix, r_ul_matrix, r_lu_index, r_ul_index

    def _setup_collision_rate_matrix(self, coll_exc_coeff, coll_deexc_coeff, no_of_levels, shell, n_e):
        coll_deexc_coeff.index = coll_deexc_coeff.index.swaplevel(0,1)

        index = self._get_rate_index(no_of_levels)
        coll_excitation_rates = (coll_exc_coeff.ix[index, shell]).fillna(0).values.reshape(
            (no_of_levels, no_of_levels)) * n_e
        coll_deexcitation_rates = (coll_deexc_coeff.ix[index, shell]).fillna(0).values.reshape(
            (no_of_levels, no_of_levels)) * n_e
        ...
        diag_deexc[coll_deexc_lvl_sum.index.astype(int)] = coll_deexc_lvl_sum.values

        diag = - np.diag(diag_exc + diag_deexc) * n_e

        collision_rate_matrix = coll_deexcitation_rates + coll_excitation_rates + diag
