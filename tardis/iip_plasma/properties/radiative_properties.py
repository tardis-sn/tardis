...
from tardis.plasma.properties.base import ProcessingPlasmaProperty, ConvergedPlasmaProperty
from tardis.plasma.properties.util import macro_atom

logger = logging.getLogger(__name__)

__all__ = ['StimulatedEmissionFactor', 'TauSobolev', 'BetaSobolev',
    'TransitionProbabilities', 'LTEJBlues', 'TauSobolevDeriv']
...
class TransitionProbabilities(ConvergedPlasmaProperty):
    ...
    def calculate(self, atomic_data, beta_sobolev, j_blues,
        ...
        transition_probabilities = self._calculate_transition_probability(macro_atom_data, beta_sobolev, j_blues,
    ...
    def calculate_transition_probabilities(self, macro_atom_data, beta_sobolev, j_blues, stimulated_emission_factor):
        transition_probabilities = self.prepare_transition_probabilities(macro_atom_data, beta_sobolev, j_blues,
                                                                         stimulated_emission_factor)
        return transition_probabilities
...
class TauSobolevDeriv(ProcessingPlasmaProperty):
    ...
    def calculate(self, lines, lines_lower_level_index, lines_upper_level_index,
                  time_explosion, f_lu, wavelength_cm, g):
        g_lower = np.array(g.ix[lines_lower_level_index], dtype=np.float64)
        g_upper = np.array(g.ix[lines_upper_level_index], dtype=np.float64)
