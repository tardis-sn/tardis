...
__all__ = ['SpontRecombRateCoeff', 'PhotoIonRateCoeff', 'StimRecombRateCoeff', 'CollIonRateCoeff',
           'CollRecombRateCoeff', 'ThermalBalanceTest', 'BfHeatingRateCoeff', 'StimRecombCoolingRateCoeff',
           'CollExcRateCoeff', 'CollDeexcRateCoeff', 'BfHeatingRateCoeffEstim',
           'Logger', 'CollExcCooling', 'Yg', 'YgInterpolator']
...
def calculate_rate_coefficient_from_estimator(estimator, photo_ion_index_sorted):
    ...
    rate_coeff = pd.DataFrame(estimator, index=index,
            columns=np.arange(no_of_shells), copy=True)
    return rate_coeff
...
class PhotoIonRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate(self, photo_ion_estimator, photo_ion_index_sorted, previous_t_electrons, photo_ion_cross_sections, previous_b):
        ...
        if photo_ion_estimator is not None:
            ...
            gamma_Ly_new = self._update_Lymann_continuum(gamma, photo_ion_cross_sections, previous_t_electrons, previous_b)
        ...
        return gamma, gamma_Ly_old, gamma_Ly_new, self.gamma_Ly_analytic_evolution, self.gamma_Ly_estim_evolution

    def calculate_from_radiation_field_model(self, photo_ion_cross_sections, w, t_rad):
        j_nus = self._calculate_j_nus(photo_ion_cross_sections, w, t_rad)
        photoion_coeff = j_nus.multiply(4. * np.pi * photo_ion_cross_sections['x_sect'] /
                                                  photo_ion_cross_sections['nu'] / const.h.cgs.value, axis=0)
    ...
    def _update_Lymann_continuum(self, gamma, photo_ion_cross_sections, t_electrons, previous_b):
        x_sect_Ly = photo_ion_cross_sections.loc[(1, 0, 0)]
        #print "Previous b:", previous_b.loc[(1,0,0)]
        W = (previous_b.loc[(1,0,0)].values) ** -1
        gamma_Ly = self.calculate_from_radiation_field_model(x_sect_Ly, W, t_electrons)
...
class BfHeatingRateCoeffEstim(ProcessingPlasmaProperty):
    ...
    def calculate(self, bf_heating_estimator, photo_ion_index_sorted):
        if bf_heating_estimator is not None:
            bf_heating_coeff = calculate_rate_coefficient_from_estimator(bf_heating_estimator, photo_ion_index_sorted)
            if self.plasma_parent.niter > self.plasma_parent.niter_ly:
...
class BfHeatingRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate_from_radiation_field_model(self, photo_ion_cross_sections, w, t_rad):
        ...
        photoion_heating_coeff = j_nus.multiply(4. * np.pi * photo_ion_cross_sections['x_sect'] * (1 - nu_is /
                                                  photo_ion_cross_sections['nu']), axis=0)
        ...
        tmp = {}
        for i in range(len(w)):
            tmp[i] = photoion_heating_coeff.apply(lambda sub: simps(sub[i], sub['nu'], even='first'))
...
class StimRecombCoolingRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate(self, stim_recomb_cooling_estimator, photo_ion_index_sorted):
        if stim_recomb_cooling_estimator is not None:
            coeff = calculate_rate_coefficient_from_estimator(stim_recomb_cooling_estimator, photo_ion_index_sorted)
            if self.plasma_parent.niter > self.plasma_parent.niter_ly:
                coeff.loc[(1,0,0)] = 0.0
        else:
            coeff = None
...
class StimRecombRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate_from_radiation_field_model(self, photo_ion_cross_sections, w, t_rad, stim_recomb_estimator, t_electrons, phi_lucy):
        j_nus = self._calculate_j_nus(photo_ion_cross_sections, w, t_rad)
        photoion_coeff = j_nus.multiply(4. * np.pi * photo_ion_cross_sections['x_sect'] /
                                                  photo_ion_cross_sections['nu'] / const.h.cgs.value, axis=0)
        boltzmann_factor = np.exp(-photo_ion_cross_sections.nu.values[np.newaxis].T / \
                                  t_electrons * (const.h.cgs.value / const.k_B.cgs.value))
...
class CollIonRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate(self, photo_ion_cross_sections, t_electrons):
        ...
        collion_coeff = photo_ion_cross_sections.groupby(level=[0, 1, 2]).first()
        factor = self._calculate_factor(collion_coeff['nu'].values, t_electrons, index=collion_coeff.index)
    ...
    def _calculate_factor(self, nu_ijk, t_electrons, index):
        ...
        factor = pd.DataFrame(factor, index=index, columns=np.arange(len(t_electrons)))
        return factor
...
class YgInterpolator(ProcessingPlasmaProperty):
    ...
    def calculate(self, Yg_data, T_Yg, lines_multi_index):
        ...
        interp = PchipInterpolator(T_Yg, Yg_data, axis=1,
                extrapolate=True)
...
class CollExcRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate(self, lines, excitation_energy, t_electrons, yg,
                  yg_allowed_index, yg_forbidden_index):
        I_H = cconst.I_H
        ...
        coll_excitation_coeff = coll_excitation_coeff.dot(pd.DataFrame(np.sqrt(t_electrons)).T)
        u0 = nu_lines[np.newaxis].T / t_electrons * (const.h.cgs.value / const.k_B.cgs.value)
        ...
        index_array = [
            lines_filtered.atomic_number.values, lines_filtered.ion_number.values,
            lines_filtered.level_number_lower.values, lines_filtered.level_number_upper.values
        ]

        index = pd.MultiIndex.from_arrays(index_array)

        coll_excitation_coeff.set_index(index, inplace=True)

        # Calculate rates based on PB04
        ind = yg.index
        ll_index = pd.MultiIndex.from_arrays(
            [ind.get_level_values(0), ind.get_level_values(1), ind.get_level_values(2)]
        )
        lu_index = pd.MultiIndex.from_arrays(
            [ind.get_level_values(0), ind.get_level_values(1), ind.get_level_values(3)]
        )
        ...
        boltzman_factor = pd.DataFrame(np.exp(-hnu.values / (const.k_B.cgs.value
            * t_electrons.T)), index=yg.index)
        q_ij = 8.629e-6 / np.sqrt(t_electrons) * yg * boltzman_factor.loc[yg.index]
...
class CollDeexcRateCoeff(ProcessingPlasmaProperty):
    ...
    def calculate(self, lte_level_boltzmann_factor_Te, coll_exc_coeff):
        ...
        index_lower = pd.MultiIndex.from_arrays([atom_number, ion_number, level_number_lower])
        index_upper = pd.MultiIndex.from_arrays([atom_number, ion_number, level_number_upper])
...
class CollExcCooling(ProcessingPlasmaProperty):
    ...
    def calculate(self, coll_exc_coeff, coll_deexc_coeff, level_number_density,
            electron_densities, nu_lines_coll):
        coll_exc_cooling_coeff = coll_exc_coeff.multiply(nu_lines_coll *
                const.h.cgs.value, axis=0)
        coll_exc_cooling_coeff = coll_exc_cooling_coeff.multiply(electron_densities, axis=1)

        coll_deexc_heating_coeff = coll_deexc_coeff.multiply(nu_lines_coll *
                const.h.cgs.value, axis=0)
        coll_deexc_heating_coeff = coll_deexc_heating_coeff.multiply(electron_densities, axis=1)


        lower_index = coll_exc_coeff.index.droplevel(level=3)
        upper_index = coll_exc_coeff.index.droplevel(level=2)

        coll_exc_cooling = coll_exc_cooling_coeff.multiply(level_number_density.loc[lower_index].values)
        coll_deexc_heating = coll_deexc_heating_coeff.multiply(level_number_density.loc[upper_index].values)

        return coll_exc_cooling.sum(), coll_deexc_heating.sum()

from level_population import PhiLucy
from partition_function import PartitionFunction, LevelBoltzmannFactorLTETe
from general import BetaRadiation, GElectron
from ion_population import PhiSahaElectrons
...
class ThermalBalanceTest(ProcessingPlasmaProperty):
    ...
    def __init__(self, plasma_parent, T_min=2500, T_max=1.e5):
    ...
    def calculate(self, photo_ion_cross_sections, t_rad, t_electrons, previous_t_electrons, excitation_energy, g, levels, ionization_data,
                  ff_heating_estimator, bf_heating_coeff, stim_recomb_cooling_coeff, coll_deexc_heating_estimator,
                  ion_number_density, level_number_density, electron_densities,
                  lines, w, time_explosion, previous_b, coll_exc_cooling, coll_deexc_heating):
        ...
        if ff_heating_estimator is not None:
            ...
            for shell in range(len(electron_densities)):
                ...
                args = (ff_heating_estimator, bf_heating_coeff, stim_recomb_cooling_coeff,
                                coll_deexc_heating_estimator, electron_densities, ion_number_density,
                                    level_number_density, excitation_energy, g, levels, ionization_data,
                                        photo_ion_cross_sections, lines, shell,
                                        t_rad, w, time_explosion, previous_b,
                                        previous_t_electrons, coll_exc_cooling, coll_deexc_heating)

                heat_mid, frac_heating, sp_fb_cooling_rate = self.heating_function(t_old, *args)
    ...
    @staticmethod
    def _calculate_phi_lucy(t_electrons, excitation_energy, g, levels, ionization_data):
        beta_electron = BetaRadiation.calculate(np.array([t_electrons]))
        level_boltzmann_factor = LevelBoltzmannFactorLTETe.calculate(excitation_energy, g, beta_electron, levels)
        partition_function = PartitionFunction.calculate(level_boltzmann_factor)
        g_electron_Te = GElectron.calculate(beta_electron)
        phi_Te = PhiSahaElectrons.calculate(g_electron_Te, beta_electron, partition_function, ionization_data)
        phi_lucy = PhiLucy(plasma_parent=None).calculate(phi_Te, level_boltzmann_factor, partition_function)
        return phi_lucy[0]

    def _calculate_ff_heating_balance(self, t_electron, ff_heating_estimator, electron_densities, ion_number_density, shell):
        ff_heating_factor = self._calculate_ff_heating_factor(ion_number_density[shell], electron_densities[shell])
        # TODO
        #heating_balance = (ff_heating_estimator[shell] / np.sqrt(t_electron) - cconst.C0_ff * np.sqrt(t_electron)) \
        #                  * ff_heating_factor
        ff_heating = ff_heating_estimator[shell] / np.sqrt(t_electron) * ff_heating_factor
    ...
    def _calculate_ff_heating_factor(ion_number_density, electron_densities):
        ionic_charge_squared = np.square(ion_number_density.index.get_level_values(1).values)
        factor = electron_densities * ion_number_density.multiply(ionic_charge_squared, axis=0).sum()
        return factor


    def _calculate_bf_heating_rate(self, bf_heating_coeff, level_number_density, shell, t_electron, photo_ion_cross_sections, b, previous_t_electrons):
        ...
        return bf_heating.multiply(level_number_density.loc[bf_heating.index, shell]).sum()

    def _calculate_adiabatic_cooling(self, electron_density, t_electron, time_explosion, shell):
        ...
        ad_cool =  3. * electron_density * const.k_B.cgs.value * t_electron / time_explosion
        return ad_cool

    def _calculate_fb_cooling_rate(self, t_electron, stim_recomb_cooling_coeff, phi_lucy, electron_densities,
                                   ion_number_density, photo_ion_cross_sections, shell):
        alpha_E_sp = self._calculate_sp_recomb_heating_rate_coeff(t_electron, photo_ion_cross_sections)
        
        if self.plasma_parent.niter > self.plasma_parent.niter_ly:
            alpha_E_sp.loc[(1,0,0)] = 0.0

        recomb_cooling_coeff = alpha_E_sp
        level_cooling_rates = recomb_cooling_coeff.multiply(phi_lucy.loc[recomb_cooling_coeff.index])\
                       * electron_densities[shell]
        ion_index = get_ion_multi_index(level_cooling_rates.index)
        level_cooling_rates = level_cooling_rates.multiply(ion_number_density.loc[ion_index, shell].values)
        ...
        st_level_cooling_rates = st_recomb_cooling_coeff.multiply(phi_lucy.loc[st_recomb_cooling_coeff.index])\
                       * electron_densities[shell]
        ion_index = get_ion_multi_index(st_level_cooling_rates.index)
        st_level_cooling_rates = st_level_cooling_rates.multiply(ion_number_density.loc[ion_index, shell].values)
        st_cooling_rate = st_level_cooling_rates.sum()

        return cooling_rate + st_cooling_rate, ly_cooling_rate, level_cooling_rates

    def _calculate_coll_ion_heating_balance(self, t_electron, photo_ion_cross_sections, phi_lucy,
                                            electron_densities, level_number_density, ion_number_density, shell):
        electron_density = electron_densities[shell]
        coll_ion_rate_coeff = CollIonRateCoeff(
            plasma_parent=None).calculate(photo_ion_cross_sections, np.array([t_electron]))[0]
        ...
        coll_ion_cooling = level_number_density.loc[index, shell] * electron_density * coll_ion_rate_coeff
        nu_i = photo_ion_cross_sections['nu'].groupby(level=[0,1,2]).first()
        #heating_balance = (heating_balance * nu_i * const.h.cgs.value).sum()
        #return heating_balance
        return (coll_ion_heating * nu_i * const.h.cgs.value).sum(), (coll_ion_cooling * nu_i * const.h.cgs.value).sum()

    def _calculate_sp_recomb_heating_rate_coeff(self, t_electron, photo_ion_cross_sections):
        x_sect = photo_ion_cross_sections['x_sect']
        nu = photo_ion_cross_sections['nu']
        nu_i = photo_ion_cross_sections['nu'].groupby(level=[0,1,2]).first()
        alpha_sp_E = (8 * np.pi * x_sect * nu ** 3 * const.h.cgs.value / (const.c.cgs.value) ** 2)
        alpha_sp_E = alpha_sp_E.multiply(1. - nu_i / nu)
        boltzmann_factor = np.exp(-nu.values / t_electron * (const.h.cgs.value / const.k_B.cgs.value))
        alpha_sp_E = pd.DataFrame(alpha_sp_E.multiply(boltzmann_factor))
        alpha_sp_E.insert(0, 'nu', nu)
        alpha_sp_E = alpha_sp_E.groupby(level=[0,1,2]).apply(lambda sub:
                trapz(sub[0], sub['nu']))
        return alpha_sp_E

    def _calculate_stim_recomb_heating_rate_coeff(self, t_electron, photo_ion_cross_sections, t_rad, w):
        nus = photo_ion_cross_sections['nu'].values
        j_nus = w * intensity_black_body(nus, t_rad)
        x_sect = photo_ion_cross_sections['x_sect']
        nu = photo_ion_cross_sections['nu']
        nu_i = photo_ion_cross_sections['nu'].groupby(level=[0,1,2]).first()
        alpha_sp_E = (4. * np.pi * x_sect) * j_nus
        alpha_sp_E = alpha_sp_E.multiply(1. - nu_i / nu)
        boltzmann_factor = np.exp(-nu.values / t_electron * (const.h.cgs.value / const.k_B.cgs.value))
        alpha_sp_E = pd.DataFrame(alpha_sp_E.multiply(boltzmann_factor))
        alpha_sp_E.insert(0, 'nu', nu)
    ...
    def heating_function(self, t_electron, ff_heating_estimator, bf_heating_coeff, stim_recomb_cooling_coeff,
                              coll_deexc_heating_estimator, electron_densities, ion_number_density,
                              level_number_density, excitation_energy, g, levels, ionization_data,
                              photo_ion_cross_sections, lines, shell, t_rad, w,
                              time_explosion, b, previous_t_electrons,
                              coll_exc_cooling, coll_deexc_heating, mode='detailed'):

        bf_heating = self._calculate_bf_heating_rate(
            bf_heating_coeff, level_number_density, shell, t_electron, photo_ion_cross_sections, b, previous_t_electrons)
        ...
        phi_lucy = self._calculate_phi_lucy(t_electron, excitation_energy, g, levels, ionization_data)
        ...
        fb_cooling, ly_cooling_rate, sp_fb_cooling_rate = self._calculate_fb_cooling_rate(
            t_electron, stim_recomb_cooling_coeff, phi_lucy, electron_densities,
                ion_number_density, photo_ion_cross_sections, shell)
...
#        #self.coll_exc_heating_evolution[self._counter][shell] = coll_exc_heating_balance
        ...
        adiabatic_cooling = self._calculate_adiabatic_cooling(electron_densities[shell], t_electron, time_explosion, shell)

        total_cooling = fb_cooling + ff_cooling + coll_ion_cooling + coll_exc_cooling[shell]
        total_heating = bf_heating + ff_heating + coll_ion_heating + coll_deexc_heating[shell]
        ...
        if not np.isfinite(conv):
            ...
            print "Shell: {}, H: {}, Q: {}".format(shell, total_heating,
                    total_cooling)
    ...
    def _update_Lymann_continuum(self, bf_heat_coeff, photo_ion_cross_sections, t_electron, b):
        W = b ** -1
        x_sect_Ly = photo_ion_cross_sections.loc[(1, 0, 0)]
        bf_heat_Ly = self.calculate_from_radiation_field_model(x_sect_Ly, np.array([W]), np.array(t_electron))
...
class Logger(ProcessingPlasmaProperty):
    ...
    def calculate(self, ion_number_density, level_number_density, t_electrons, b, gamma):
        ...
        logdata = {'ion_frac': self.ion_frac, 't_electrons': self.t_electrons, 'gamma': gamma, 'b0': self.b0}
        return logdata
