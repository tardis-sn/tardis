...
class MatplotlibWidget(FigureCanvas):
    ...
    def span_picker(self, span, mouseevent, tolerance=5):
        ...
        tolerance = span.axes.transData.inverted().transform(
            (tolerance, 0))[0] - span.axes.transData.inverted().transform((0, 0))[0]
                    ...
                    if isinstance(dict2[key][1], list):
                        ...
                        if optionselected in options:
                            ...
                            raise exceptions.IOError(
                                "An invalid option was" + " provided in the input file")
...
class ModelViewer(QtGui.QWidget):
    ...
    def make_spectrum_widget(self):
        ...
        self.spectrum_button.menu().addAction('spec_flux_angstrom').triggered.connect(
            self.change_spectrum_to_spec_flux_angstrom)
    ...
    def on_ion_header_double_clicked(self, index):
        ...
        self.levelsdata = self.createTable(
            [
                ['Level: '], [
                    'Count (Ion %d)' %
                    self.current_ion_index]], iterate_header=(
                2, 0), index_info=self.table3_data.index.values.tolist())
        normalized_data = []
...
class LineInfo(QtGui.QDialog):
    ...
    def get_data(self, wavelength_start, wavelength_end):
        ...
        self.ions_out.sort()
        self.header_list = []
        self.ion_table = (self.grouped_lines_in.wavelength.count().astype(
            float) / self.grouped_lines_in.wavelength.count().sum()).values.tolist()
    ...
    def get_transition_table(self, lines, atom, ion):
        ...
        transitions_with_duplicates = lines.ix[grouped.groups[(atom, ion)]].groupby(
            ['level_number_lower', 'level_number_upper']).groups
        transitions = lines.ix[grouped.groups[(atom, ion)]].drop_duplicates(
        ...
        for key, value in transitions.items():
            transitions_parsed.append(
                ...
                (key[0],
                 key[1],
                    self.parent.model.atom_data.lines.ix[
                    value[0]]['wavelength']))
...
class LineInteractionTables(QtGui.QWidget):
    ...
    def __init__(self, line_interaction_analysis, atom_data, description,
        ...
        species_symbols = [util.species_tuple_to_string(
            item, atom_data) for item in self.species_selected]
        ...
        self.species_table.connect(
            self.species_table.verticalHeader(),
            QtCore.SIGNAL('sectionClicked(int)'),
    ...
    def on_species_clicked(self, index):
        ...
        last_line_in_filter = (
            last_line_in.atomic_number == current_species[0]).values & (
            last_line_in.ion_number == current_species[1]).values

        current_last_line_in = last_line_in[last_line_in_filter].reset_index()
        current_last_line_out = last_line_out[
            last_line_in_filter].reset_index()
